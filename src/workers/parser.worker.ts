...
const buildLexicon = (ALL_UNITS: MasterUnit[]) => {
  ...
  DICTIONARY = Array.from(LEXICON_MAP.values()).sort(
...
const extractQuantityAndClean = (
  rawToken: string
): { qty: number; cleanText: string } => {
  let text = rawToken.replace(
    /\b(my|ur|your|his|hers|hes|he's|their|accept|taking|wanted|any good offers|good offers?|offers?|offer|traid|trade|trading|upgrading|downgrading|adds?|good|bad|idk|overpay|op|worth|nty|fair|any|s tier|a tier|b tier|c tier|oddities)\b/gi,
    " "
  );
  let qty = 1;
  ...
  if (frontXMatch) {
    ...
    if (backXMatch) {
      ...
      const frontNumMatch = text.match(
        /^(\d+)\s+(?!(?:lab|star|\*|eye|k|%|stardust|gems?|egg)\b)(.+)$/i
      );
      if (frontNumMatch) {
...
const matchTokenToUnit = (
  unitText: string,
  ALL_UNITS: MasterUnit[],
  slangCache: Record<string, string>
): ParsedItem | null => {
  ...
  if (exactEntry) {
    return {
      ...
      confidence:
        exactEntry.type === "exact"
          ? 100
          : exactEntry.type === "alias"
          ? 95
          : 85,
    };
  }
  ...
  const scored = DICTIONARY.map((entry) => {
    ...
    if (inputWords.every((w) => targetWords.includes(w))) {
      const lengthPenalty =
        Math.abs(entry.key.length - normalized.length) * 0.1;
      score = Math.min(score, 1 + lengthPenalty);
    }
    return { entry, score };
  })
    .filter((e) => e.score <= Math.max(2, normalized.length * 0.3))
    .sort((a, b) => a.score - b.score);

  if (scored.length > 0) {
    const bestDist = scored[0].score;
    const closeMatches = scored
      .filter((e) => e.score <= bestDist + 0.5)
      .flatMap((e) => e.entry.units);
    const uniqueMatches = Array.from(
      new Map(closeMatches.map((u) => [u.id, u])).values()
    );
    return {
      rawName: unitText,
      qty: 1,
...
self.onmessage = (e) => {
  ...
  if (!text || text.length > 5000) {
    self.postMessage({
      ...
      result: {
        ...
        ambiguous: [],
        error:
          "Input too large (max 5000 characters). Please paste a smaller trade block to prevent browser lockup.",
      },
    });
    return;
  }
  ...
  const hwMatch = text.match(
    /\b(?:have|h|give|giving)\b\s*[:\-]?\s*(.*?)\s*\b(?:want|w|lf|looking for|get|getting)\b\s*[:\-]?\s*(.*)/i
  );
  if (hwMatch) {
    givePart = hwMatch[1];
    getPart = hwMatch[2];
    ...
    const forMatch = text.match(
      /(.*?)\s+(?:for|want|wants|gets|looking for|lf|->|=>)\s+(.*)/i
    );
    if (forMatch) {
      givePart = forMatch[1];
      getPart = forMatch[2];
  ...
  const parseChunk = (
    chunkText: string,
    col: "give" | "get"
  ): { cards: TradeCard[]; ambiguous: AmbiguousToken[] } => {
    ...
    for (const item of parsedItems) {
      const uniqueOptions = Array.from(
        new Map(item.options.map((u) => [u.id, u])).values()
      );
      if (uniqueOptions.length === 1 && item.confidence >= 60) {
        const unit = uniqueOptions[0];
        const numericValue =
          typeof unit.value === "number" ? unit.value : unit.valueMin || 0;
  ...
  const getRes = getPart
    ? parseChunk(getPart, "get")
    : { cards: [], ambiguous: [] };
  ...
  self.postMessage({
    ...
    result: {
      ...
      error:
        totalCount === 0 && totalAmbiguous === 0
          ? "No matching units found. Check for typos."
          : null,
    },
  });
};
