...
const TradeAnalyzerPanel = lazy(() =>
  import("./components/TradeAnalyzer").then((module) => ({
    default: module.TradeAnalyzerPanel,
  }))
);
const Sidebar = lazy(() =>
  import("./components/Sidebar").then((module) => ({ default: module.Sidebar }))
);
const MainCanvas = lazy(() =>
  import("./components/MainCanvas").then((module) => ({
    default: module.MainCanvas,
  }))
);
const HomeChannel = lazy(() =>
  import("./components/HomeChannel").then((module) => ({
    default: module.HomeChannel,
  }))
);
const TutorialChannel = lazy(() =>
  import("./components/TutorialChannel").then((module) => ({
    default: module.TutorialChannel,
  }))
);
const InventoryChannel = lazy(() =>
  import("./components/InventoryChannel").then((module) => ({
    default: module.InventoryChannel,
  }))
);
const TradingAdsChannel = lazy(() =>
  import("./components/TradingAdsChannel").then((module) => ({
    default: module.TradingAdsChannel,
  }))
);
const ExtraNoticesChannel = lazy(() =>
  import("./components/ExtraNoticesChannel").then((module) => ({
    default: module.ExtraNoticesChannel,
  }))
);
const LegalChannel = lazy(() =>
  import("./components/LegalChannel").then((module) => ({
    default: module.LegalChannel,
  }))
);
const AdminChannel = lazy(() =>
  import("./components/AdminChannel").then((module) => ({
...
const CHANNEL_INFO: Record<string, { title: string; subtitle: string }> = {
  home: {
    title: "home",
    subtitle:
      "Welcome to the ASTD Value List! Important information and update logs are posted here.",
  },
  "value-list": {
    title: "value-list",
    subtitle: "ASTD unit values - Being Observed Live by Fire Zio",
  },
  "trading-ads": {
    title: "trading-ads",
    subtitle: "Live community trade listings - Direct Discord messaging",
  },
  tutorial: {
    title: "tutorial",
    subtitle: "Learn how to use the ASTD trading calculator and value list.",
  },
  inventory: {
    title: "my-inventory",
    subtitle: "Manage your personal unit collection and vault.",
  },
  profile: {
    title: "user-profile",
    subtitle: "Trader Identity and Public Records",
  },
  "extra-notices": {
    title: "extra-notices",
    subtitle: "Additional rules, exceptions, and community notes.",
  },
  "terms-of-service": {
    title: "terms-of-service",
    subtitle: "Rules and guidelines for using the ASTD Value List.",
  },
  "privacy-policy": {
  ...
  "admin-panel": {
    title: "admin-panel",
    subtitle: "Moderation and User Management Database.",
  },
};
...
export default function App() {
  ...
  >("sandbox");
  const [activeTierFilter, setActiveTierFilter] = useStickyState<FilterKey>(
    "S",
    "astd_tier"
  );
  ...
  const [isRosterOpen, setIsRosterOpen] = useStickyState(
    window.innerWidth >= 768,
    "astd_roster",
    isBoolean
  );
  const [isAnalyzerOpen, setIsAnalyzerOpen] = useStickyState(
    false,
    "astd_analyzer",
    isBoolean
  );
  const [loginModalChannel, setLoginModalChannel] = useState<string | null>(
    null
  );
  ...
  useEffect(() => {
    if (
      !isAuthLoading &&
      !profile &&
      (activeChannel === "inventory" || activeChannel === "trading-ads")
    ) {
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("start-guest-tour", handleStartGuest);
  }, [startGuide]);
  ...
  const { handleTouchStart, handleTouchEnd } = useMobileSwipe(
    isRosterOpen,
    setIsRosterOpen
  );

  const activeItemsCount =
    giveItems.reduce((acc, c) => acc + c.qty, 0) +
    getItems.reduce((acc, c) => acc + c.qty, 0);
  const isDictionaryActive =
    activeChannel === "tutorial" && tutorialTab === "dictionary";
  ...
  const handleThreadClick = useCallback(
    (tier: FilterKey, sectionId: string) => {
  ...
  const currentChannelInfo = CHANNEL_INFO[activeChannel] || {
    title: activeChannel,
    subtitle: "",
  };
  ...
  const isGuestStep2 =
    guideState.type === "guest_tour" && guideState.step === 2;
  const isGuestStep3 =
    guideState.type === "guest_tour" && guideState.step === 3;
  const isGuestStep4 =
    guideState.type === "guest_tour" && guideState.step === 4;

  const sidebarZ = isGuestStep1
    ? "!z-[100000] shadow-[15px_0_50px_rgba(0,0,0,0.8)] relative"
    : "z-50";
  const mainContentZ = isGuestStep2
    ? "!z-[100000] relative shadow-[0_0_50px_rgba(0,0,0,0.8)]"
    : "z-auto";
  const calcHeaderZ = isGuestStep3
    ? "!z-[99999] shadow-[0_0_50px_rgba(0,0,0,0.8)] relative"
    : "z-50";
  const analyzerZ = isGuestStep4
    ? "!z-[100000] shadow-[-20px_0_50px_rgba(0,0,0,0.8)]"
    : "z-50";
  ...
  if (bootStage === "complete" && profile?.role === "banned") {
    return (
      <div className="flex flex-col items-center justify-center w-screen h-dvh bg-background text-foreground font-sans p-6 text-center select-none">
        ...
        <h1 className="text-[28px] font-black uppercase tracking-widest text-destructive mb-2">
          Account Terminated
        </h1>
        <p className="text-muted-foreground text-[14px] max-w-md leading-relaxed mb-8">
          Your access to the ASTD Value List platform has been permanently
          revoked by the moderation team.
        </p>
        ...
        <a
          ...
          className="px-8 py-3.5 bg-primary hover:bg-primary/90 text-primary-foreground font-bold text-[13px] uppercase tracking-wider rounded-[4px] flex items-center justify-center gap-2 border border-border focus-visible:outline-none"
        >
    ...
    <>
      ...
      >
        <Suspense fallback={null}>
          ...
          <div className="fixed bottom-[140px] md:bottom-8 left-1/2 -translate-x-1/2 pointer-events-none transition-all flex flex-col gap-2 items-center z-[9999]">
            <div
              ...
              {toast && (
                <div className="flex items-center gap-3 px-5 py-3.5 rounded-[4px] border border-border bg-card shadow-sm">
                  ...
                  <span className="text-foreground text-[13px] font-bold tracking-wide whitespace-nowrap">
                    Added{" "}
                    <strong className="font-black">{toast.unitName}</strong>
                    {toast.count > 1 && (
          ...
          <div
            className={`fixed md:relative top-0 bottom-0 left-0 flex-shrink-0 overflow-hidden transition-transform shadow-2xl md:shadow-none bg-card z-30 ${
              isRosterOpen
                ? "w-[85vw] max-w-[260px] md:w-[240px] translate-x-0"
          ...
          >
            ...
            >
              <Routes>
                <Route
                  path="/home"
                  element={<HomeChannel guideState={guideState} />}
                />
                <Route
                  path="/tutorial"
                  element={
                ...
                <Route
                  path="/extra-notices"
                  element={<ExtraNoticesChannel />}
                />
                <Route
                  path="/terms-of-service"
                  element={<LegalChannel type="tos" />}
                />
                <Route
                  path="/privacy-policy"
                  element={<LegalChannel type="privacy" />}
                />
                    ...
                    <div className="flex-1 flex items-center justify-center bg-background px-4">
                      <div className="text-center">
                        ...
                        <p className="text-muted-foreground">
                          This channel is currently under construction or does
                          not exist.
                        </p>
                      </div>
                    </div>
