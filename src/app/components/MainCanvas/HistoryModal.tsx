...
export function HistoryModal() {
  ...
  const CustomizedDot = (props: any) => {
    ...
    if (isMax) {
      return (
        <g>
          <circle cx={cx} cy={cy} r={7} fill="#43b581" stroke="#111214" strokeWidth={3} />
    ...
    if (isMin) {
      return (
        <g>
          <circle cx={cx} cy={cy} r={7} fill="#ed4245" stroke="#111214" strokeWidth={3} />
  ...
  return (
    <div className="fixed inset-0 z-[1000000] flex items-center justify-center p-0 sm:p-5 animate-fade-in">
      ...
      <div className="bg-[#18191C] border border-[rgba(255,255,255,0.08)] rounded-none sm:rounded-[24px] w-full h-full sm:h-auto sm:max-w-5xl sm:max-h-[94vh] flex flex-col relative shadow-[0_24px_64px_rgba(0,0,0,0.8)] overflow-hidden animate-slide-up">
        
        {/* Solid Tier-Colored Top Accent Bar */}
        <div className="absolute top-0 left-0 right-0 h-[4px] sm:h-[5px] z-[200]" style={{ background: tierCfg.badgeColor }} />

        {/* Stable Header Layout */}
        <div className={`px-5 sm:px-6 pt-6 pb-4 shrink-0 z-[100] bg-[#1E1F22] transition-all duration-300 ${isScrolled ? 'shadow-md border-b border-[rgba(255,255,255,0.08)]' : 'border-b border-transparent'}`}>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4 sm:gap-5 min-w-0">
              ...
              <div className="flex flex-col min-w-0 justify-center">
                <div className="flex items-center gap-2.5 flex-wrap">
                  <h2 className="text-[#F2F3F5] text-[20px] sm:text-[24px] font-black tracking-tight truncate drop-shadow-sm">
                    {currentUnit.name}
                  </h2>
                  {statusCfg && (
                    <span className="text-[10px] font-extrabold uppercase px-2.5 py-0.5 rounded-full border shadow-sm tracking-wider" style={{ backgroundColor: statusCfg.bg, color: statusCfg.color, borderColor: statusCfg.border }}>
                      {statusCfg.label}
                    </span>
                  )}
                </div>
                ...
                <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                  ...
                  <span className="text-[10px] font-black uppercase px-2 py-0.5 rounded-[6px] bg-[#5865F2]/20 text-[#5865F2] border border-[#5865F2]/40 shadow-sm">
                    Tier {tierKey}
                  </span>
                </div>
              </div>
            ...
            <button 
              onClick={closeModal} 
              className="text-[#949BA4] hover:text-[#F2F3F5] bg-[rgba(255,255,255,0.06)] hover:bg-[rgba(255,255,255,0.12)] p-2.5 rounded-full transition-all duration-200 focus-visible:outline-none shrink-0 self-start shadow-md active:scale-95"
            >
              <X className="w-5 h-5" />
            </button>
          </div>
        ...
        {/* Scrollable Body - Split Layout on Desktop */}
        <div 
          ref={scrollRef}
        ...
        >
          <div className="p-5 sm:p-6 flex flex-col lg:flex-row gap-6 lg:gap-8">
            ...
            {/* Left Column: Metrics & Notices */}
            <div className="w-full lg:w-[320px] flex flex-col gap-4 sm:gap-6 shrink-0">
              <div className="grid grid-cols-2 gap-3">
                <div 
                  onClick={() => setActiveMetric('value')}
                  className={`col-span-2 rounded-[16px] p-4 flex flex-col justify-center cursor-pointer transition-all duration-300 relative overflow-hidden group ${activeMetric === 'value' ? 'bg-[#2B2D31] ring-2 ring-[#5865F2] shadow-[0_8px_20px_rgba(88,101,242,0.25)]' : 'bg-[#1E1F22] hover:bg-[#2B2D31] border border-[rgba(255,255,255,0.04)]'}`}
                >
                ...
                <div 
                  ...
                  className={`rounded-[16px] p-4 flex flex-col justify-center cursor-pointer transition-all duration-300 relative overflow-hidden group ${activeMetric === 'rarity' ? 'bg-[#2B2D31] ring-2 ring-[#4DB6AC] shadow-[0_8px_20px_rgba(77,182,172,0.25)]' : 'bg-[#1E1F22] hover:bg-[#2B2D31] border border-[rgba(255,255,255,0.04)]'}`}
                >
                ...
                <div 
                  ...
                  className={`rounded-[16px] p-4 flex flex-col justify-center cursor-pointer transition-all duration-300 relative overflow-hidden group ${activeMetric === 'supply' ? 'bg-[#2B2D31] ring-2 ring-[#81C784] shadow-[0_8px_20px_rgba(129,199,132,0.25)]' : 'bg-[#1E1F22] hover:bg-[#2B2D31] border border-[rgba(255,255,255,0.04)]'}`}
                >
                ...
                <div 
                  ...
                  className={`col-span-2 rounded-[16px] p-4 flex flex-col justify-center cursor-pointer transition-all duration-300 relative overflow-hidden group ${activeMetric === 'demand' ? 'bg-[#2B2D31] ring-2 ring-[#FFB74D] shadow-[0_8px_20px_rgba(255,183,77,0.25)]' : 'bg-[#1E1F22] hover:bg-[#2B2D31] border border-[rgba(255,255,255,0.04)]'}`}
                >
              ...
              {(currentUnit.notice || (currentUnit.secondaryTags && currentUnit.secondaryTags.length > 0)) && (
                <div className="bg-[#1E1F22] border border-[rgba(255,255,255,0.06)] rounded-[16px] p-4.5 flex flex-col gap-3 shadow-inner">
                  {currentUnit.notice && (
                    <div className="flex flex-col gap-2">
                      <span className="flex w-fit items-center gap-1.5 text-[10px] font-black uppercase tracking-widest px-2.5 py-1 rounded-[6px] bg-[#5865F2] text-white shadow-sm shrink-0">
                        <AlertCircle className="w-3.5 h-3.5" /> Notice
                      </span>
                      <p className="text-[13px] font-medium text-[#DBDEE1] leading-relaxed">{currentUnit.notice}</p>
                    </div>
                  )}
                    ...
                    <div className="flex flex-col gap-2 pt-2 border-t border-[rgba(255,255,255,0.04)] mt-1">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {currentUnit.secondaryTags.map(tag => (
                          <span key={tag} className="text-[10.5px] font-black uppercase px-2.5 py-1 rounded-[6px] bg-[#111214] text-[#F2F3F5] border border-[rgba(255,255,255,0.08)] shadow-sm">
                            {tag}
                          </span>
                        ))}
                      </div>
            ...
            {/* Right Column: Chart & Audit Log */}
            <div className="flex-1 flex flex-col gap-6 lg:gap-8 min-w-0">
              
              {loading ? (
                <div className="h-[280px] flex flex-col items-center justify-center text-[#949BA4] animate-pulse gap-3 font-medium text-sm bg-[#1E1F22] rounded-[20px] border border-[rgba(255,255,255,0.04)]">
                  <Sparkles className="w-6 h-6 animate-spin text-[#5865F2]" />
                  Syncing historical snapshots...
                </div>
              ) : error ? (
                <div className="text-[#ed4245] text-sm text-center py-8 bg-[#1E1F22] rounded-[20px] border border-[rgba(237,66,69,0.2)]">Failed to load history: {error}</div>
              ) : history.length <= 1 ? (
                <div className="flex flex-col items-center justify-center h-[280px] text-center bg-[#1E1F22] border border-[rgba(255,255,255,0.06)] rounded-[20px] shadow-lg">
                  <TrendingUp className="w-12 h-12 text-[#949BA4] mb-4 opacity-60 animate-bounce" />
                  <p className="text-[#F2F3F5] font-black text-[16px]">Fire Zio is looking...</p>
                  <p className="text-[#949BA4] text-[13px] mt-2 max-w-sm px-6 leading-relaxed">Baseline snapshot recorded for {currentUnit.name}. Trend charts and logs will generate as market shifts happen.</p>
                </div>
              ) : (
                <div className="bg-[#1E1F22] border border-[rgba(255,255,255,0.06)] p-4 sm:p-5 rounded-[20px] flex flex-col gap-4 shadow-xl">
                  <div className="flex items-center justify-between flex-wrap gap-3">
                    <h3 className="text-[#F2F3F5] text-[13.5px] font-black uppercase tracking-wider flex items-center gap-2 shrink-0">
                      <BarChart2 className="w-4 h-4" style={{ color: currentConfig.color }} /> {currentConfig.label}
                    </h3>
                    <div className="flex items-center gap-3">
                      <div className="hidden sm:flex items-center gap-1.5 text-[10px] font-bold text-[#949BA4]">
                  ...
                  <div className="h-[240px] sm:h-[280px] w-full bg-[#111214]/60 p-2 sm:p-4 rounded-[14px] border border-[rgba(255,255,255,0.04)]">
                    <ResponsiveContainer width="100%" height="100%">
                      <AreaChart data={chartData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                        ...
                        <XAxis dataKey="date" stroke="#949BA4" fontSize={11} tickLine={false} axisLine={false} tickMargin={8} />
                        ...
                        <Tooltip 
                          contentStyle={{ backgroundColor: '#18191C', border: '1px solid rgba(255,255,255,0.12)', borderRadius: '12px', boxShadow: '0 12px 32px rgba(0,0,0,0.7)', padding: '10px 14px' }}
                        ...
                        />
                        <Area type="monotone" dataKey="value" stroke={currentConfig.color} strokeWidth={3} fillOpacity={1} fill={`url(#${currentConfig.gradientId})`} dot={<CustomizedDot />} activeDot={{ r: 9, fill: currentConfig.color, stroke: '#111214', strokeWidth: 3 }} />
                      </AreaChart>
                    </ResponsiveContainer>
                  </div>
                </div>
              ...
              {timeline.length > 0 && (
                <div className="flex flex-col gap-4 mt-2">
                  <h3 className="text-[#949BA4] text-[13px] font-black uppercase tracking-widest flex items-center gap-2">
                    <History className="w-4 h-4 text-[#5865F2]" /> Audit Timeline
                  </h3>
                  
                  <div className="relative pl-6 sm:pl-8 ml-2 border-l-2 border-[rgba(255,255,255,0.06)] flex flex-col gap-6 sm:gap-8 pb-4">
                    {timeline.map((t, idx) => {
                      ...
                      return (
                        <div key={idx} className="relative group">
                          {/* Timeline Node */}
                          <div className="absolute -left-[31px] sm:-left-[39px] top-1.5 w-3 h-3 bg-[#5865F2] rounded-full ring-4 ring-[#18191C] group-hover:scale-125 transition-transform" />
                          
                          <div className="bg-[#1E1F22] border border-[rgba(255,255,255,0.06)] rounded-[16px] p-4 sm:p-5 flex flex-col gap-4 shadow-sm hover:border-[rgba(255,255,255,0.12)] transition-colors">
                            <div className="flex items-center justify-between border-b border-[rgba(255,255,255,0.06)] pb-3 flex-wrap gap-2">
                              <span className="text-[#DBDEE1] text-[12px] font-mono font-black">{t.date}</span>
                              <div className="flex gap-1.5 flex-wrap">
                                {t.changed.map((c: string) => (
                                  <span key={c} className="text-[9px] sm:text-[10px] font-black bg-[#111214] text-[#F2F3F5] px-2.5 py-0.5 rounded-[6px] border border-[rgba(255,255,255,0.08)] uppercase tracking-wider shadow-sm">
                                    {c}
                                  </span>
                                ))}
                              </div>
                            ...
                            <div className="flex flex-col gap-3">
                              {t.changed.includes("Value") && (
                                <div className="flex items-center justify-between bg-[#111214] px-3.5 py-2.5 rounded-[10px] border border-[rgba(255,255,255,0.03)] flex-wrap gap-2">
                                  <span className="text-[12px] text-[#949BA4] font-bold">Value Shift</span>
                                  <div className="flex items-center gap-3 font-mono text-[13px] sm:text-[14px] font-black">
                                    <span className="text-[#949BA4] line-through decoration-[#ed4245]">{t.prev.value_display || (t.prev.value ?? 0).toLocaleString()}</span>
                              ...
                              {t.changed.includes("Status") && (
                                <div className="flex items-center justify-between bg-[#111214] px-3.5 py-2.5 rounded-[10px] border border-[rgba(255,255,255,0.03)] flex-wrap gap-2">
                                  ...
                                  <div className="flex items-center gap-3 text-[11px] sm:text-[12px] font-bold">
                              ...
                              {(t.changed.includes("Rarity") || t.changed.includes("Supply") || t.changed.includes("Demand")) && (
                                <div className="flex items-center justify-between bg-[#111214] px-3.5 py-2.5 rounded-[10px] border border-[rgba(255,255,255,0.03)] text-[12px] flex-wrap gap-2">
                                  <span className="text-[#949BA4] font-bold">Attributes Shift</span>
                                  <div className="flex gap-4 font-mono text-[12px] font-bold text-[#DBDEE1]">
