...
export function HistoryModal() {
  ...
  if (!loading && !error) {
    ...
    const currentValNum =
      typeof currentUnit.value === "number"
        ? currentUnit.value
        : currentUnit.valueMin || 0;

    const isLiveDifferent =
      !latestDbSnap ||
      (latestDbSnap.value_type === "range"
        ? latestDbSnap.value_min
        : latestDbSnap.value) !== currentValNum ||
      latestDbSnap.status !== currentUnit.status ||
      latestDbSnap.rarity !== currentUnit.rarity ||
      (latestDbSnap.liquidity || "Average") !==
        (currentUnit.liquidity || "Average");
    ...
    if (isLiveDifferent) {
      displayHistory.push({
        ...
        value_type:
          typeof currentUnit.value === "number" ? "number" : currentUnit.value,
  ...
  const latestSnap: HistorySnapshot | undefined =
    displayHistory[displayHistory.length - 1];

  const metricConfigs = {
    value: {
      label: "Value Trend History",
      color: "var(--primary)",
      gradientId: "valueGrad",
      unitLabel: "Value",
    },
    rarity: {
      label: "Rarity Trend History",
      color: "#4DB6AC",
      gradientId: "rarityGrad",
      unitLabel: "Rarity",
    },
    liquidity: {
      label: "Liquidity Trend History",
      color: "#81C784",
      gradientId: "liqGrad",
      unitLabel: "Liquidity",
    },
  ...
  const maxVal =
    validChartData.length > 0
      ? Math.max(...validChartData.map((d) => d.value))
      : 0;
  const minVal =
    validChartData.length > 0
      ? Math.min(...validChartData.map((d) => d.value))
      : 0;

  const oldestSnap = displayHistory[0];
  const oldestSnapVal = oldestSnap
    ? oldestSnap.value_type === "range"
      ? oldestSnap.value_min
      : oldestSnap.value
    : null;
  const latestSnapVal = latestSnap
    ? latestSnap.value_type === "range"
      ? latestSnap.value_min
      : latestSnap.value
    : null;
  const pctChange =
    oldestSnapVal && latestSnapVal && oldestSnapVal > 0
  ...
  for (let i = displayHistory.length - 1; i > 0; i--) {
    ...
    const changes = [];
    if (curr.value !== prev.value || curr.value_display !== prev.value_display)
      changes.push("Value");
    if (curr.status !== prev.status) changes.push("Status");
    if (curr.rarity !== prev.rarity) changes.push("Rarity");
    if ((curr.liquidity || "Average") !== (prev.liquidity || "Average"))
  ...
  const statusCfg =
    GRID_STATUS_CFG[
      (latestSnap?.status || currentUnit.status) as keyof typeof GRID_STATUS_CFG
    ];
  ...
  return (
    <div className="fixed inset-0 z-[1000000] flex items-center justify-center p-0 sm:p-5 animate-fade-in">
      ...
      <div className="bg-card border border-border rounded-none sm:rounded-[12px] w-full h-full sm:h-auto sm:max-w-5xl sm:max-h-[94vh] flex flex-col relative shadow-2xl overflow-hidden animate-slide-up">
        <div
          className="absolute top-0 left-0 right-0 h-[4px] z-[200]"
          style={{ background: tierCfg.badgeColor }}
        />

        <div
          className={`px-5 sm:px-6 pt-6 pb-4 shrink-0 z-[100] bg-popover transition-all duration-300 ${
            isScrolled
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4 sm:gap-5 min-w-0">
              <div className="relative shrink-0">
                <div className="relative overflow-hidden bg-black/20 flex items-center justify-center border border-border w-20 h-20 sm:w-24 sm:h-24 rounded-[8px]">
                  <div
                    className="absolute inset-0 flex items-center justify-center text-white font-black text-xl sm:text-2xl z-0"
                    style={getAvatarStyle(currentUnit.name)}
                  >
              ...
              <div className="flex flex-col min-w-0 justify-center">
                <div className="flex items-center gap-2.5 flex-wrap">
                  ...
                  {statusCfg && (
                    <span
                      className="text-[10px] font-bold uppercase px-2 py-0.5 rounded-[4px] border tracking-wider inline-flex items-center gap-1"
                      style={{
                ...
                <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                  <span className="text-muted-foreground text-[11px] sm:text-[12px] font-bold uppercase tracking-wider">
                    {currentUnit.subtitle || "Official Unit"}
                  </span>
            ...
            <div className="w-full lg:w-[300px] flex flex-col gap-3 shrink-0">
              <div className="grid grid-cols-2 gap-3">
                <div
                  ...
                  className={`col-span-2 rounded-[8px] p-4 flex flex-col justify-center cursor-pointer transition-all active:scale-[0.98] relative overflow-hidden group ${
                    activeMetric === "value"
                      ? "bg-card border border-primary"
                      : "bg-popover hover:bg-card border border-border"
                  }`}
                >
                  {activeMetric === "value" && (
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    {displayHistory.length > 1 &&
                      ...
                      oldestSnapVal > 0 && (
                        <span
                          className={`text-[10px] font-bold px-1.5 py-0.5 rounded-[3px] flex items-center gap-0.5 ${
                            pctChange >= 0
                              ? "bg-[#23a559]/10 text-[#23a559]"
                  ...
                  <span className="text-[18px] sm:text-[20px] font-black font-mono text-foreground mt-1.5 truncate">
                    {currentUnit.value === "owner" ||
                    currentUnit.valueDisplay === "Owner's Choice"
                      ? "Owner's Choice"
                      : currentUnit.valueDisplay ||
                        (currentUnit.value as number).toLocaleString()}
                  </span>
                </div>
                ...
                <div
                  onClick={() => {
                    triggerHaptic("light");
                    setActiveMetric("rarity");
                  }}
                  className={`rounded-[8px] p-4 flex flex-col justify-center cursor-pointer transition-all active:scale-[0.98] relative overflow-hidden group ${
                    activeMetric === "rarity"
                      ? "bg-card border border-[#4DB6AC]"
                      : "bg-popover hover:bg-card border border-border"
                  }`}
                >
                  {activeMetric === "rarity" && (
                  ...
                  <span className="text-[16px] font-black font-mono text-[#4DB6AC] mt-1.5">
                    {latestSnap?.rarity ?? currentUnit.rarity ?? 0}
                  </span>
                </div>
                ...
                <div
                  onClick={() => {
                    triggerHaptic("light");
                    setActiveMetric("liquidity");
                  }}
                  className={`rounded-[8px] p-4 flex flex-col justify-center cursor-pointer transition-all active:scale-[0.98] relative overflow-hidden group ${
                    activeMetric === "liquidity"
                      ? "bg-card border border-[#81C784]"
                      : "bg-popover hover:bg-card border border-border"
                  }`}
                >
                  {activeMetric === "liquidity" && (
                  ...
                  <span className="text-[13px] font-black font-mono text-[#81C784] mt-1.5 uppercase">
                    {latestSnap?.liquidity ??
                      currentUnit.liquidity ??
                      "Average"}
                  </span>
                </div>
              </div>
                      ...
                      <div className="flex flex-col gap-2 pt-2 border-t border-border mt-1">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {currentUnit.secondaryTags.map((tag) => (
                            <span
                              key={tag}
                              className="text-[10px] font-bold uppercase px-2 py-0.5 rounded-[4px] bg-black/20 text-foreground border border-border"
                            >
                              {tag}
                            </span>
                          ))}
            ...
            <div className="flex-1 flex flex-col gap-6 min-w-0">
              {loading ? (
                ...
                <div className="flex flex-col items-center justify-center h-[280px] text-center bg-popover border border-border rounded-[8px]">
                  ...
                  <p className="text-muted-foreground text-[12px] mt-1 max-w-sm px-6 leading-relaxed">
                    Baseline snapshot recorded for {currentUnit.name}. Trend
                    charts will generate as market shifts happen.
                  </p>
                </div>
              ) : (
                  ...
                  <div className="flex items-center justify-between flex-wrap gap-3">
                    ...
                    <div className="flex items-center gap-3">
                      <div className="hidden sm:flex items-center gap-1.5 text-[10px] font-bold text-muted-foreground uppercase">
                        <span className="w-2 h-2 rounded-[2px] bg-[#23a559] inline-block" />{" "}
                        Peak
                        <span className="w-2 h-2 rounded-[2px] bg-destructive inline-block ml-1" />{" "}
                        Low
                      </div>
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <div className="relative pl-6 ml-2 border-l border-border flex flex-col gap-4 pb-4">
                    {timeline.map((t, idx) => {
                      const currCfg =
                        GRID_STATUS_CFG[
                          t.snap.status as keyof typeof GRID_STATUS_CFG
                        ];
                      const prevCfg =
                        GRID_STATUS_CFG[
                          t.prev.status as keyof typeof GRID_STATUS_CFG
                        ];
                        ...
                        <div key={idx} className="relative group">
                          ...
                          <div className="bg-popover border border-border rounded-[8px] p-4 flex flex-col gap-3 hover:border-muted-foreground transition-colors">
                            <div className="flex items-center justify-between border-b border-border pb-2 flex-wrap gap-2">
                              <span className="text-muted-foreground text-[11px] font-mono font-bold uppercase tracking-wider">
                                {t.date}
                              </span>
                              <div className="flex gap-1.5 flex-wrap">
                                {t.changed.map((c: string) => (
                                  <span
                                    key={c}
                                    className="text-[9px] font-bold bg-black/20 text-foreground px-2 py-0.5 rounded-[4px] border border-border uppercase tracking-wider"
                                  >
                                    {c}
                                  </span>
                                ))}
                                ...
                                <div className="flex items-center justify-between bg-black/20 px-3 py-2 rounded-[6px] border border-border flex-wrap gap-2">
                                  ...
                                  <div className="flex items-center gap-3 font-mono text-[12px] font-bold">
                                    <span className="text-muted-foreground line-through decoration-destructive">
                                      {t.prev.value_display ||
                                        (t.prev.value ?? 0).toLocaleString()}
                                    </span>
                                    <span className="text-primary">➔</span>
                                    <span className="text-foreground">
                                      {t.snap.value_display ||
                                        (t.snap.value ?? 0).toLocaleString()}
                                    </span>
                                  </div>
                                </div>
                              )}
                              ...
                              {t.changed.includes("Status") && (
                                <div className="flex items-center justify-between bg-black/20 px-3 py-2 rounded-[6px] border border-border flex-wrap gap-2">
                                  ...
                                  <div className="flex items-center gap-3 text-[10px] font-bold tracking-wider">
                                    <span
                                      className="px-2 py-0.5 rounded-[4px] border uppercase inline-flex items-center gap-1"
                                      style={{
                                        backgroundColor:
                                          prevCfg?.bg || "var(--card)",
                                        color:
                                    ...
                                    <span
                                      ...
                                      style={{
                                        backgroundColor:
                                          currCfg?.bg || "var(--card)",
                                        color:
                                          currCfg?.color || "var(--foreground)",
                                        borderColor:
                                          currCfg?.border || "var(--border)",
                                      }}
                                    >
                              ...
                              {t.changed.includes("Liquidity") && (
                                <div className="flex items-center justify-between bg-black/20 px-3 py-2 rounded-[6px] border border-border flex-wrap gap-2">
                                  ...
                                  <div className="flex gap-3 font-mono text-[11px] font-bold text-foreground uppercase">
                                    <span className="text-muted-foreground">
                                      {t.prev.liquidity || "Average"}
                                    </span>{" "}
                                    ➔{" "}
                                    <span className="text-[#81C784]">
                                      {t.snap.liquidity || "Average"}
                                    </span>
                                  </div>
                                </div>
                              )}
