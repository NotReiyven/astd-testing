...
export const ListHeaderRow = memo(function ListHeaderRow({
  ...
  const getSortIcon = (key: string) => {
    if (sortMode === `${key}-desc`)
      return <ArrowDown className="w-3.5 h-3.5 text-foreground ml-1" />;
    if (sortMode === `${key}-asc`)
      return <ArrowUp className="w-3.5 h-3.5 text-foreground ml-1" />;
    return null;
  };

  return (
    <div
      className={`hidden md:grid ${getGridCols(
        isCompact
      )} items-stretch bg-card text-muted-foreground text-[10px] font-bold uppercase tracking-widest select-none w-full border-y border-border sticky top-0 z-20`}
    >
      {!isCompact && (
        <div className="px-3 py-3 flex items-center justify-center">Icon</div>
      )}
      <div className="px-3 py-3 flex items-center justify-start border-l border-border">
        Units
      </div>

      <button
        onClick={() => handleSort("value")}
        className={`px-3 py-3 flex items-center justify-end border-l border-border transition-colors hover:bg-muted cursor-pointer focus-visible:outline-none ${
          sortMode.includes("value")
            ? "text-foreground bg-popover"
      ...
      <button
        ...
        title="Rarity (0-20)"
        className={`px-2 py-3 flex items-center justify-center border-l border-border transition-colors hover:bg-muted cursor-pointer focus-visible:outline-none ${
          sortMode.includes("rarity")
            ? "text-foreground bg-popover"
            : "hover:text-foreground"
        }`}
      >
...
export const UnitListRow = memo(function UnitListRow({
  ...
  const obtainability = (() => {
    ...
    if (
      lowerNotice.includes("(unobtainable)") ||
      lowerNotice.includes("[unobtainable]")
    )
      return "UNOB";
    if (
      lowerNotice.includes("(obtainable)") ||
      lowerNotice.includes("[obtainable]")
    )
      return "OBN";
    ...
    if (
      unit.tier === "Oddities" ||
      subCat.includes("gamepass") ||
      lowerName.includes("premium pass") ||
      lowerName.includes("star pass")
    )
      return "OBN";
    if (
      unit.tier === "C" &&
      lowerNotice.includes("banner") &&
      !lowerName.includes("snowman")
    )
      return "OBN";
    if (lowerNotice.includes("capsule")) return "OBN";

    if (/\bobtainable\b/.test(lowerNotice.replace(/unobtainable/g, "")))
      return "OBN";
  ...
  const valDisplay =
    unit.tier === "Untiered" ? (
      <span className="text-[13px] md:text-[14px] font-bold tracking-tight text-muted-foreground font-mono">
        N/A
      </span>
    ) : unit.value === "owner" ||
      unit.valueDisplay === "Owner's Choice" ||
      unit.valueDisplay === "O/C" ? (
      <span className="text-[12px] font-bold tracking-tight bg-foreground text-background px-1.5 py-0.5 rounded-[2px] font-mono">
        <JargonWrap
      ...
      <span className="text-[12.5px] md:text-[13.5px] font-bold tracking-tight text-foreground font-mono">
        {unit.valueDisplay}
      </span>
    ) : (
      <span className="text-[13px] md:text-[14px] font-bold tracking-tight text-foreground font-mono">
  ...
  const liqDisplay =
    liqString.toLowerCase() === "black marketed"
      ? "BM"
      : liqString.substring(0, 3).toUpperCase();
    ...
    <>
      ...
      >
        <div
          ...
          className={`relative flex flex-col md:grid ${getGridCols(
            isCompact
          )} items-stretch cursor-pointer select-none bg-card hover:bg-muted z-10 will-change-transform transition-colors duration-100`}
        >
          {/* ICON COLUMN */}
          {!isCompact && (
          ...
          <div
            className={`hidden md:flex flex-col justify-center min-w-0 px-3 border-r border-border bg-card ${
              isCompact ? "py-1" : "py-2"
            }`}
          >
              ...
              <div className="flex items-center gap-1.5 mt-1.5">
                {obtainability === "UNOB" ? (
                  <span className="text-[10px] font-bold uppercase text-muted-foreground bg-popover px-1.5 py-[2px] rounded-[2px] border border-border tracking-widest leading-none">
                    <JargonWrap
                      title="Unobtainable (UNOB)"
                      tip="This unit can no longer be acquired through normal gameplay. Trading is the only way to get it."
                    >
                      UNOB
                    </JargonWrap>
                  </span>
                  ...
                  <span className="text-[10px] font-bold uppercase text-foreground bg-popover px-1.5 py-[2px] rounded-[2px] border border-border tracking-widest leading-none">
                    <JargonWrap
                      title="Obtainable (OBN)"
                      tip="This unit can still be acquired in-game through summons, capsules, or evolution."
                    >
                      OBN
                    </JargonWrap>
                  </span>
          ...
          <div
            className={`hidden md:flex px-3 border-r border-border items-center justify-end relative ${
              isCompact ? "py-1" : "py-2"
            }`}
            style={{
          ...
          <div
            className={`hidden md:flex px-2 border-r border-border bg-card items-center justify-center font-mono font-bold text-[12px] ${
              isCompact ? "py-1" : "py-2"
            }`}
          ...
          <div
            ...
            style={{ color: getStatColor("L", liqString) }}
          >
            {liqDisplay === "BM" ? (
              <JargonWrap
                title="Black Marketed (BM)"
                tip="This unit's value is heavily manipulated by outside-game currency trades. Highly risky."
              >
                BM
              </JargonWrap>
            ) : (
          ...
          <div
            className={`hidden md:flex px-3 items-center min-w-0 bg-card relative ${
              isCompact ? "py-1" : "py-2"
            }`}
          >
            {unit.notice ? (
              <span className="text-[11.5px] font-medium text-muted-foreground line-clamp-2 leading-snug">
                {unit.notice}
              </span>
            ) : (
              <span className="text-[11.5px] font-medium text-muted-foreground italic">
                No notes
              </span>
            )}
          </div>
          ...
          <div className="flex md:hidden items-center justify-between w-full px-4 pb-3 relative min-h-[28px]">
            <div className="flex items-center gap-2 text-[12px] font-mono">
              <span className="text-muted-foreground">
                R{" "}
                <span style={{ color: getStatColor("R", unit.rarity) }}>
                  {unit.rarity}
                </span>
              </span>
              ...
              <span className="text-muted-foreground">
                L{" "}
                <span style={{ color: getStatColor("L", liqString) }}>
                  {liqDisplay === "BM" ? (
                    <JargonWrap
                      title="Black Marketed (BM)"
                      tip="This unit's value is heavily manipulated by outside-game currency trades. Highly risky."
            ...
            {unit.notice && (
              <div className="flex-1 px-3 text-[11px] text-muted-foreground italic leading-snug truncate z-10">
                {unit.notice}
              </div>
            )}
          </div>
