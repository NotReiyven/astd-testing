...
export const ListHeaderRow = memo(function ListHeaderRow({ sortMode, setSortMode, viewMode }: { sortMode: string, setSortMode: (s: string) => void, viewMode: string }) {
  ...
  return (
    <div className={`hidden md:grid ${getGridCols(isCompact)} items-stretch bg-card text-muted-foreground text-[10px] font-black uppercase tracking-widest select-none w-full border-y border-border shadow-sm sticky top-0 z-20`}>
    ...
    <>
      <div className={`relative w-full overflow-hidden bg-card ${isLast ? '' : 'border-b border-border'}`}>
        <div
          ...
          className={`relative flex flex-col md:grid ${getGridCols(isCompact)} items-stretch cursor-pointer select-none bg-card hover:bg-muted z-10 will-change-transform transition-colors duration-150`}
        >
          {/* ICON COLUMN */}
          {!isCompact && (
            <div className="hidden md:flex px-3 py-2 items-center justify-center border-r border-border">
              <div className="relative w-8 h-8 rounded-full overflow-hidden bg-popover border border-border shadow-sm flex-shrink-0 flex items-center justify-center">
                ...
                <img 
                  ...
                  className="absolute inset-0 w-full h-full object-cover animate-fade-in z-10 bg-popover" 
                />
              </div>
            </div>
          )}
          ...
          <div className="flex md:hidden items-center justify-between w-full px-4 py-3">
            <div className="flex items-center gap-3 min-w-0 pr-2">
              <div className="relative w-10 h-10 rounded-[6px] overflow-hidden bg-popover border border-border shadow-sm flex-shrink-0 flex items-center justify-center">
          ...
          <div className={`hidden md:flex flex-col justify-center min-w-0 px-3 border-r border-border ${isCompact ? 'py-1' : 'py-2'}`}>
            ...
            {!isCompact && (
              <div className="flex items-center gap-1.5 mt-1.5">
                {obtainability === "UNOB" ? (
                  <span className="text-[10px] font-bold uppercase text-muted-foreground bg-popover px-1.5 py-[2px] rounded-[3px] border border-border tracking-widest leading-none">
                  ...
                  <span className="text-[10px] font-bold uppercase text-foreground bg-popover px-1.5 py-[2px] rounded-[3px] border border-border tracking-widest leading-none">
          ...
          <div 
            className={`hidden md:flex px-3 border-r border-border items-center justify-end relative transition-colors ${isCompact ? 'py-1' : 'py-2'}`}
            style={{
          ...
          <div className={`hidden md:flex px-2 border-r border-border items-center justify-center font-mono font-bold text-[12px] ${isCompact ? 'py-1' : 'py-2'}`} style={{ color: getStatColor("R", unit.rarity) }}>
            {unit.rarity}
          </div>
          <div className={`hidden md:flex px-2 border-r border-border items-center justify-center font-mono font-bold text-[12px] ${isCompact ? 'py-1' : 'py-2'}`} style={{ color: getStatColor("L", liqString) }}>
            {liqDisplay === "BM" ? (
