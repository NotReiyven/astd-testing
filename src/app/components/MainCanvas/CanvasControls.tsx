...
export function CanvasControls({
  ...
  const renderViewModeToggles = () => (
    <div className="flex bg-popover rounded-[6px] p-[3px] border border-border flex-shrink-0 shadow-sm">
      <button
        ...
        className={`p-2 rounded-[4px] transition-all duration-200 ease-out active:scale-95 cursor-pointer focus-visible:outline-none min-h-[40px] min-w-[40px] flex items-center justify-center ${
          viewMode === "grid"
      ...
      <button
        ...
        className={`hidden md:flex p-2 rounded-[4px] transition-all duration-200 ease-out active:scale-95 cursor-pointer focus-visible:outline-none min-h-[40px] min-w-[40px] items-center justify-center ${
          viewMode === "compact"
            ? "bg-muted text-foreground shadow-sm"
            : "text-muted-foreground hover:text-foreground"
        }`}
  ...
  return (
    <div
      className={`flex-shrink-0 flex flex-col z-40 relative bg-card border-b border-border shadow-sm ${
        globalCompactMode
          ? "px-2 md:px-4 py-2 gap-2"
          : "px-4 md:px-10 py-4 gap-3.5"
      }`}
    >
        ...
        {FILTERS.map((f) => (
          <button
            ...
            onClick={() => {
              ...
              if (f !== "All")
                window.dispatchEvent(new Event("academy-used-filter"));
            }}
      ...
      <div
        className={`flex flex-col transition-all duration-300 overflow-hidden ${
          isControlsCollapsed
            ? "max-h-0 opacity-0 md:max-h-none md:opacity-100 py-0 gap-0 md:gap-3"
            : "max-h-[400px] opacity-100 gap-3"
        }`}
      >
        {/* desktop layout */}
        <div
          className={`hidden md:flex flex-row items-center justify-between gap-3 w-full border-t border-border/60 ${
            globalCompactMode ? "pt-2" : "pt-3"
          }`}
        >
        ...
        {/* mobile layout */}
        <div
          className={`flex md:hidden flex-col gap-3 w-full border-t border-border/60 ${
            globalCompactMode ? "pt-2" : "pt-3"
          }`}
        >
