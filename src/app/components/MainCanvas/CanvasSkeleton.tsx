    ...
    return (
      <div className="w-full rounded-[6px] border border-border bg-card shadow-sm overflow-hidden">
        {[...Array(isCompact ? 15 : 8)].map((_, i) => (
          <div
            key={i}
            className={`animate-shimmer flex md:grid ${
              isCompact
                ? "md:grid-cols-[minmax(200px,1fr)_160px_120px]"
                : "md:grid-cols-[60px_minmax(200px,1.2fr)_140px_60px_70px_minmax(200px,2fr)]"
            } items-stretch border-b border-border ${
              isCompact ? "min-h-[32px] px-4 py-1.5" : "min-h-[60px] px-4 py-2"
            } gap-4`}
          >
            {!isCompact && (
            ...
            <div className="flex flex-col justify-center gap-2">
              ...
              {!isCompact && (
                <div className="h-2 w-1/2 bg-border rounded-[2px]" />
              )}
            </div>
  ...
  return (
    ...
    >
      {[...Array(12)].map((_, i) => (
        <div
          key={i}
          className="flex flex-col h-[280px] md:h-[340px] rounded-[6px] bg-card border border-border overflow-hidden animate-shimmer"
        >
