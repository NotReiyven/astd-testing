...
export function GridStatBox({
  ...
  if (type === "rarity") {
    ...
    if (numVal >= 19) textColor = "#4DB6AC";
    else if (numVal >= 9) textColor = "#81C784";
    else if (numVal >= 6) textColor = "#FFB74D";
    else textColor = "#E57373";
  } else {
    const stringVal = String(value);
    const liqKey =
      stringVal.charAt(0).toUpperCase() + stringVal.slice(1).toLowerCase();
    displayValue =
      ...
      {tipPos &&
        createPortal(
          <>
            ...
            />
            <div
              className="rounded-[8px] px-3 py-2.5 pointer-events-none fixed z-[99999] -translate-x-1/2 -translate-y-full animate-fade-in shadow-[0_8px_24px_rgba(0,0,0,0.6)]"
              style={{
                top: tipPos.y,
                left: tipPos.x,
                minWidth: 200,
            ...
            >
              ...
              <p className="text-[11px] font-medium leading-snug text-foreground whitespace-normal">
                {tipBody}
              </p>
            </div>
          </>,
