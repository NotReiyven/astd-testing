  ...
  return createPortal(
    ...
    >
      {!profile ? (
        ...
        <>
          <div className="h-[50px] w-full relative border-b border-border" style={{ backgroundColor: profile.banner_color || '#121214' }} />
          ...
          <div className="px-4 pb-4 relative">
            <div className="absolute -top-8 left-4 cursor-pointer hover:opacity-80 transition-opacity" onClick={handleViewFullProfile} title="View Full Profile">
              <div className="relative">
                <img 
                  ...
                  className="w-16 h-16 rounded-[4px] border-4 border-popover object-cover bg-muted"
                />
                <div 
            ...
            <div className="mt-2 flex flex-col bg-card p-3 rounded-[4px] border border-border shadow-inner">
              <h2 
                className="text-[16px] font-black text-foreground tracking-tight leading-none mb-1 truncate cursor-pointer hover:underline"
              ...
              <div className="flex items-center gap-2 mt-2">
                <div className="flex items-center gap-1.5 text-muted-foreground bg-muted px-2 py-0.5 rounded-[3px] border border-border cursor-pointer hover:text-foreground transition-colors group" onClick={handleCopyDiscord}>
                  <span className="text-[11px] font-mono font-bold truncate max-w-[120px]">{profile.discord_id}</span>
                  {copied ? <Check className="w-3 h-3 text-[#23a559]" /> : <Copy className="w-3 h-3 group-hover:text-foreground" />}
                </div>

                <div className={`flex items-center gap-1 px-2 py-0.5 rounded-[3px] border text-[11px] font-bold ${profile.global_rep > 0 ? 'bg-[#23a559]/10 border-[#23a559]/30 text-[#23a559]' : profile.global_rep < 0 ? 'bg-destructive/10 border-destructive/30 text-destructive' : 'bg-muted border-border text-muted-foreground'}`}>
            ...
            <div className="mt-3 flex gap-2">
              <button
                onClick={handleViewFullProfile}
                className="flex-1 flex items-center justify-center gap-2 py-2 rounded-[4px] bg-muted hover:bg-card border border-border text-foreground text-[12px] font-bold transition-colors focus-visible:outline-none cursor-pointer"
              >
                <UserCircle className="w-3.5 h-3.5" /> Full Profile
              </button>
              <button
                onClick={handleContact}
                className="flex-1 flex items-center justify-center gap-2 py-2 rounded-[4px] bg-[#5865F2] hover:bg-[#4752C4] text-white text-[12px] font-bold transition-colors shadow-sm focus-visible:outline-none cursor-pointer"
              >
