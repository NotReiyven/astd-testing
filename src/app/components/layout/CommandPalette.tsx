...
const COMMANDS = [
  {
    ...
    action: "value-list",
    type: "navigate",
    desc: "View the live market data",
  },
  {
    id: "nav-ads",
    label: "Trading Ads",
    ...
    action: "trading-ads",
    type: "navigate",
    desc: "View live community listings",
  },
  {
    id: "nav-inv",
    label: "My Inventory",
    ...
    action: "inventory",
    type: "navigate",
    desc: "Manage your vault and wishlist",
  },
  {
    id: "nav-home",
    label: "Home & Patch Notes",
    icon: Hash,
    action: "home",
    type: "navigate",
    desc: "View recent updates",
  },
  {
    id: "tool-calc",
    ...
    action: "toggle-calc",
    type: "action",
    desc: "Calculate trade values",
  },
  {
    id: "acad-sandbox",
    label: "Academy: Checklist",
    icon: GraduationCap,
    action: "sandbox",
    type: "academy",
    desc: "View your certification progress",
  },
  {
    id: "acad-mock",
    label: "Academy: Mock Trades",
    icon: Map,
    action: "simulator",
    type: "academy",
    desc: "Test your trading knowledge",
  },
  {
    id: "acad-theory",
    label: "Academy: Market Theory",
    icon: BookOpen,
    action: "theory",
    type: "academy",
    desc: "Read the official documentation",
  },
  {
    id: "acad-dict",
    label: "Academy: Dictionary",
    icon: Settings2,
    action: "dictionary",
    type: "academy",
    desc: "Manage the smart parser",
  },
];
  ...
  const executeCommand = (cmd: (typeof COMMANDS)[0]) => {
    ...
    if (cmd.type === "navigate") {
      window.document.dispatchEvent(
        new CustomEvent("navigate", { detail: cmd.action })
      );
    } else if (cmd.type === "action") {
      window.dispatchEvent(new Event("open-analyzer"));
    } else if (cmd.type === "academy") {
      window.document.dispatchEvent(
        new CustomEvent("navigate", { detail: "tutorial" })
      );
      setTimeout(
        () =>
          window.dispatchEvent(
            new CustomEvent("set-tutorial-tab", { detail: cmd.action })
          ),
        50
      );
    }
    ...
    >
      ...
      <div className="bg-card w-full max-w-[600px] rounded-[12px] border border-border shadow-[0_20px_60px_rgba(0,0,0,0.8)] flex flex-col relative z-10 overflow-hidden animate-slide-up">
        <div className="flex items-center px-4 py-4 border-b border-border gap-3">
          ...
          />
          <span className="text-[10px] font-mono text-muted-foreground bg-popover px-1.5 py-0.5 rounded border border-border pointer-events-none">
            ESC
          </span>
        </div>
        ...
        >
          {filtered.length === 0 ? (
            <div
              className="p-6 text-center text-muted-foreground text-[13px]"
              role="option"
              aria-selected="false"
            >
            ...
            filtered.map((cmd, idx) => {
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-3 py-3 rounded-[8px] transition-colors text-left focus-visible:outline-none ${
                    isSelected
