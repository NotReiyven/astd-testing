...
import { Book, Search, Trash2, Check, AlertCircle, Plus } from "lucide-react";
...
export function DictionaryTab() {
  ...
  return (
    <div className="animate-fade-in pb-8 max-w-4xl mx-auto flex flex-col h-full gap-6 font-sans select-none">
      ...
      <div className="bg-[#1E1F22] border border-[#5865F2]/30 rounded-[12px] p-6 md:p-8 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="flex flex-col gap-2 max-w-xl">
          <div className="flex items-center gap-2">
            <div className="w-10 h-10 rounded-[8px] bg-[rgba(88,101,242,0.1)] border border-[rgba(88,101,242,0.2)] flex items-center justify-center shadow-inner shrink-0">
              <Book className="w-5 h-5 text-[#5865F2]" />
            </div>
            <h2 className="text-[20px] font-black text-[#F2F3F5] tracking-tight">Personal Dictionary</h2>
          </div>
          <p className="text-[#B5BAC1] text-[13.5px] leading-relaxed mt-2">
            The Smart Parser relies on this local dictionary to understand your specific abbreviations. If the calculator keeps misunderstanding a unit you trade frequently, link its slang here.
          </p>
        </div>
      </div>
      ...
      <div className="bg-[#111214] border-l-4 border-l-[#ed4245] border-y border-y-[rgba(255,255,255,0.04)] border-r border-r-[rgba(255,255,255,0.04)] rounded-r-[8px] p-4 shadow-inner flex items-start gap-4">
         ...
         <div className="flex flex-col gap-1">
           ...
           <p className="text-[#949BA4] text-[13px] italic font-medium leading-relaxed">
             "This data never touches our servers. It is saved directly to your browser's local storage. If you clear your browser cache, you will lose your custom slang."
           </p>
         </div>
      </div>

      <div className="flex flex-col gap-4 bg-[#2B2D31] border border-[rgba(255,255,255,0.04)] rounded-[12px] p-5 md:p-6 shadow-md">
        ...
        <div className="flex flex-col md:flex-row gap-3 relative">
          <div className="flex-1 flex flex-col gap-2">
            ...
            <input 
              ...
              className="w-full bg-[#1E1F22] border border-[rgba(255,255,255,0.04)] rounded-[6px] px-4 py-3 text-[14px] text-[#F2F3F5] outline-none placeholder-[#80848E] focus:ring-2 focus:ring-[#5865F2] transition-all shadow-inner" 
            />
          </div>
          ...
          <div className="flex-1 flex flex-col gap-2 relative" ref={dropdownRef}>
            ...
            <div className={`flex items-center bg-[#1E1F22] rounded-[6px] px-3 py-3 transition-all border shadow-inner ${isDropdownOpen ? 'border-[#5865F2] ring-1 ring-[#5865F2]' : 'border-[rgba(255,255,255,0.04)]'}`}>
            ...
            {isDropdownOpen && (
              <div className="absolute top-[105%] left-0 right-0 max-h-[300px] overflow-y-auto custom-scrollbar bg-[#2B2D31] border border-[rgba(255,255,255,0.08)] rounded-[8px] shadow-2xl z-50 flex flex-col p-2 gap-1">
                    ...
                    filteredUnits.map(u => {
                        ...
                        return (
                            <button
                                ...
                                className="flex items-center gap-3 w-full p-2.5 hover:bg-[#1E1F22] rounded-[6px] transition-colors text-left group"
                            >
                                <div className="w-10 h-10 rounded-[6px] bg-[#111214] overflow-hidden shrink-0 flex items-center justify-center border border-[rgba(255,255,255,0.04)]">
                                    {proxyUrl ? (
          ...
          <div className="flex flex-col justify-end pt-2 md:pt-0">
            <button 
              ...
              className={`flex items-center justify-center gap-2 h-[46px] px-6 rounded-[6px] text-[14px] font-bold transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white ${newSlangKey.trim() && newSlangTargetId ? "bg-[#23a559] hover:bg-[#1f914e] text-white shadow-md active:scale-95" : "bg-[#1E1F22] text-[#80848E] cursor-not-allowed border border-[rgba(255,255,255,0.02)]"}`}
      ...
      <div className="flex flex-col gap-4">
        <div className="flex items-center justify-between border-b border-[rgba(255,255,255,0.04)] pb-3">
           ...
           <span className="text-[11px] font-bold bg-[rgba(255,255,255,0.05)] text-[#DBDEE1] px-2.5 py-1 rounded-[4px]">{savedEntries.length} Saved</span>
        </div>

        {savedEntries.length === 0 ? (
          <div className="bg-[#1E1F22] border border-dashed border-[rgba(255,255,255,0.08)] rounded-[12px] p-10 flex flex-col items-center justify-center text-center opacity-70">
            <AlertCircle className="w-10 h-10 text-[#80848E] mb-3" />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            {savedEntries.map(([key, targetId]) => {
              ...
              return (
                <div key={key} className="flex items-center bg-[#2B2D31] p-3 rounded-[8px] border border-[rgba(255,255,255,0.04)] hover:border-[rgba(255,255,255,0.08)] transition-colors group">
                  ...
                  <button 
                    ...
                    className="text-[#80848E] hover:text-[#ed4245] p-2 transition-colors rounded-[6px] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#ed4245] hover:bg-[rgba(237,66,69,0.1)] shrink-0 ml-2"
                    title="Remove slang"
                  >
