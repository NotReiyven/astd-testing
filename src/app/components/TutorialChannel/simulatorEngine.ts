...
export const buildScenariosList = (ALL_UNITS: MasterUnit[]): Scenario[] => {
  ...
  const pickRandom = (arr: MasterUnit[]) =>
  ...
  const findPairedUnit = (
    g1: MasterUnit,
    q1: number,
    g2Cond: (u: MasterUnit) => boolean,
    ratioMin = 0.7,
    ratioMax = 1.3
  ): { unit: MasterUnit; q2: number } | null => {
    const v1 = (g1.value as number) * q1;
    const candidates = valid
      .filter((u) => u.id !== g1.id && g2Cond(u))
      .map((u) => {
  ...
  while (newScenarios.length < 10 && attempts < 500) {
    ...
    if (type === 0) {
      ...
      match = findPairedUnit(
        g1,
        q1,
        (u) => u.status === "dropping" || getLiq(u) === "low",
        0.9,
        1.4
      );
      ...
      match = findPairedUnit(
        g1,
        q1,
        (u) => u.status === "inflated" || u.status === "highballed",
        0.9,
        1.4
      );
      ...
      match = findPairedUnit(
        g1,
        q1,
        (u) => u.status === "rising" || getLiq(u) === "high",
        0.65,
        0.95
      );
      ...
      match = findPairedUnit(
        g1,
        q1,
        (u) => u.status === "stable" && u.rarity >= 13,
        0.75,
        1.1
      );
      title = "The Long-Term Play";
      desc = "Trading down in raw value for extreme scarcity and stability.";
    } else {
      g1 = getByCondition((u) => true);
      match = findPairedUnit(g1, q1, (u) => true, 0.8, 1.2);
      title = "The Standard Exchange";
      desc =
        "Evaluate the raw stats, tags, and liquidity to determine if this trade is a win.";
    }
    ...
    const giveCards: TradeCard[] = [
      {
        id: g1.id,
        name: g1.name,
        subtitle: g1.subtitle,
        value: g1.value as number,
        qty: q1,
      },
    ];
    const getCards: TradeCard[] = [
      {
        id: g2.id,
        name: g2.name,
        subtitle: g2.subtitle,
        value: g2.value as number,
        qty: q2,
      },
    ];
    ...
    newScenarios.push({
      ...
      correct: correctAns,
      forecast: fc,
      explanation: `The algorithm evaluates this as a ${
        isWin ? "WIN" : "LOSS"
      } (ST: ${fc.st > 0 ? "+" : ""}${fc.st.toFixed(1)} | LT: ${
        fc.lt > 0 ? "+" : ""
      }${fc.lt.toFixed(1)}). ${
        correctAns === "SCAM"
          ? "However, no sane trader would accept this. It's a massive downgrade or completely unrealistic structure."
          : ""
      }`,
    });
  }
