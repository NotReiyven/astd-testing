...
import { ArrowRight, Sparkles, Activity, RotateCcw, BookOpen, List, Timer, CheckCircle2, XCircle, AlertTriangle, ArrowDown } from "lucide-react";
    ...
    <div className="flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-6 w-full">
      <div className="flex items-center gap-4 flex-1 min-w-0">
        ...
        <div className="flex flex-col min-w-0 flex-1">
           <span className="text-[18px] font-bold text-foreground truncate tracking-tight">{unit.name}</span>
           <span className="text-[12px] font-semibold text-muted-foreground uppercase tracking-wider truncate mt-1">{unit.subtitle || "Official Unit"}</span>
        </div>
      </div>
      ...
      <div className="flex flex-row sm:flex-col items-center sm:items-end justify-between sm:justify-center shrink-0 gap-2 mt-4 sm:mt-0 pt-4 sm:pt-0 border-t border-border sm:border-t-0">
         <span className="text-[24px] font-black font-mono text-foreground leading-none">{totalVal.toLocaleString()}</span>
...
export function SimulatorTab() {
  ...
  const [guessResult, setGuessResult] = useState<"none" | "correct" | "incorrect" | "scam" | "timeout">("none");
  ...
  // Timer Logic
  useEffect(() => {
    if (!isSimulatorRunning || isAssessmentComplete || guessResult !== "none") return;
    ...
    return (
      <div className="animate-fade-in pb-6 max-w-4xl mx-auto font-sans select-none flex flex-col flex-1 justify-center h-full">
        <div className="bg-card border border-border rounded-[12px] p-8 md:p-12 shadow-xl flex flex-col items-center text-center my-auto">
            ...
            <h2 className="text-[24px] md:text-[32px] font-black text-foreground tracking-tight mb-2">Assessment Concluded</h2>
            <p className="text-muted-foreground text-[14px] uppercase tracking-widest mb-8 font-bold">Final Score: <span className="text-primary">{simScore.toLocaleString()}</span></p>
            
            <div className="w-full h-px bg-border my-2" />

            <p className="text-muted-foreground text-[15px] leading-relaxed mb-10 max-w-lg mt-6">
              Trading algorithms provide pure mathematical statistics, but a human trader must adapt to shifting trends. Your score reflects your ability to balance raw value metrics against liquidity and market manipulation.
            </p>
          ...
          <div className="bg-card border border-border rounded-[12px] p-8 md:p-10 shadow-lg flex flex-col gap-8 w-full my-auto max-w-2xl mx-auto">
            ...
            <div className="flex flex-col gap-6">
               ...
               <div className="bg-popover border-l-4 border-l-destructive border-y border-y-border border-r border-r-border rounded-r-[8px] p-4 shadow-inner flex items-start gap-4">
                   <img src="/units/firezio.webp" className="w-10 h-10 rounded-full border border-destructive object-cover shrink-0 bg-popover" alt="Fire Zio" />
        ...
        <div className="flex flex-col gap-6 flex-1">
          ...
          <div className="flex items-center justify-between px-6 py-4 bg-card border border-border rounded-[12px] shadow-sm">
            <div className="flex items-center gap-8">
              <div className="flex flex-col">
                <span className="text-[11px] font-bold uppercase tracking-widest text-muted-foreground mb-1">Score</span>
                <span className="text-[20px] font-black text-foreground font-mono leading-none">{simScore.toLocaleString()}</span>
              </div>
              <div className="w-px h-8 bg-border" />
              <div className="flex flex-col">
                <span className="text-[11px] font-bold uppercase tracking-widest text-muted-foreground mb-1">Streak</span>
                <span className={`text-[20px] font-black font-mono leading-none transition-colors ${simCombo > 2 ? 'text-[#22c55e]' : 'text-foreground'}`}>x{simCombo}</span>
              </div>
            </div>
            
            <div className="flex items-center gap-5">
               <div className="hidden sm:flex flex-col text-right">
                 <span className="text-[11px] font-bold uppercase tracking-widest text-muted-foreground mb-1">Question</span>
                 <span className="text-[16px] font-black text-foreground leading-none">{currentScenario + 1} / {scenarios.length}</span>
               </div>
               <div className={`flex items-center gap-2 px-4 py-2.5 rounded-[8px] shadow-inner transition-colors ${timeLeft <= 5 && guessResult === "none" ? 'bg-[#ef4444] text-white animate-pulse' : 'bg-popover border border-border text-foreground'}`}>
                 <Timer className="w-5 h-5" />
                 <span className="font-mono font-black text-[16px] tracking-wide">00:{timeLeft.toString().padStart(2, '0')}</span>
               </div>
            </div>
          </div>
          ...
          <div className="bg-card rounded-[12px] border border-border relative overflow-hidden shadow-lg flex flex-col flex-1">
            ...
            <div className="p-6 md:p-10 flex flex-col flex-1">
              <div className="flex flex-col gap-2 mb-10">
                <h3 className="text-[24px] md:text-[28px] font-black text-foreground tracking-tight">{scenarios[currentScenario].title}</h3>
                <p className="text-[15px] text-muted-foreground leading-relaxed max-w-2xl">{scenarios[currentScenario].desc}</p>
              </div>
                  ...
                  >
                    ...
                    <div className="flex flex-col flex-1 text-center md:text-left">
                      ...
                      <p className="text-[14px] text-foreground leading-relaxed mb-6">
                        {guessResult === "scam" 
                           ? `You identified this as a heavily manipulated or unreasonable trade. The algorithm calculated it as a ${scenarios[currentScenario].forecast.st >= 0 || scenarios[currentScenario].forecast.lt >= 0 ? "WIN" : "LOSS"}, but real-world market knowledge overrides pure math.` 
