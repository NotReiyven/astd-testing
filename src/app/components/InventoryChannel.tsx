...
export function InventoryChannel() {
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden h-full select-none font-sans relative bg-background">
      ...
      {isReadOnly && (
        <div className="flex items-center justify-between px-4 py-2.5 bg-foreground text-background shrink-0 z-30">
          <div className="flex items-center gap-2.5">
            <span className="text-[12px] font-bold uppercase tracking-wider bg-background text-foreground px-2 py-0.5 rounded-[2px]">Viewing</span>
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between px-3 md:px-5 py-3 gap-3">
          ...
          >
            ...
            {vaultView !== "wishlist" && (
              ...
              >
                <LockIcon className={`w-3.5 h-3.5 ${activeTierFilter === "Pinned" ? "text-background" : "text-[#FAA61A]"}`} /> Locked
              </button>
            )}
                  ...
                  {manageOpen && (
                    <div className="absolute top-full right-0 mt-2 w-[260px] bg-popover border border-border shadow-2xl rounded-[6px] p-1.5 z-50 flex flex-col">
                      {confirmClear ? (
                        ...
                        <>
                          <button onClick={() => { setImportMenuOpen(true); setManageOpen(false); setTimeout(() => importInputRef.current?.focus(), 100); }} className="w-full text-left px-3 py-2.5 rounded-[4px] text-[12.5px] font-bold text-foreground hover:bg-muted transition-colors focus-visible:outline-none flex items-center justify-between cursor-pointer">
                            Import from Text <Wand2 className="w-3.5 h-3.5 text-muted-foreground" />
                          </button>
                          <div className="w-full h-px bg-border my-1" />
                          <button onClick={() => { if (metrics.unpinnedCount > 0) setConfirmClear("unpinned"); }} disabled={metrics.unpinnedCount === 0} className="w-full text-left px-3 py-2.5 rounded-[4px] text-[12.5px] font-bold text-foreground hover:bg-muted disabled:opacity-50 disabled:cursor-not-allowed transition-colors focus-visible:outline-none flex items-center justify-between cursor-pointer">
                            Clear Unlocked <span className="text-muted-foreground font-mono font-bold text-[11px]">{metrics.unpinnedCount}</span>
                          </button>
                          <button onClick={() => { if (metrics.uniqueCount > 0) setConfirmClear("all"); }} disabled={metrics.uniqueCount === 0} className="w-full text-left px-3 py-2.5 rounded-[4px] text-[12.5px] font-bold text-destructive hover:bg-destructive/10 disabled:opacity-50 disabled:cursor-not-allowed transition-colors focus-visible:outline-none flex items-center justify-between cursor-pointer">
                            Clear Entire Vault <span className="text-destructive opacity-70 font-mono font-bold text-[11px]">{metrics.uniqueCount}</span>
                          </button>
                        </>
                      )}
                    </div>
                  ...
                  {unownedSearchResults.map((u, i) => {
                    ...
                    return (
                      <button
                        ...
                        className={`flex items-center gap-2.5 w-full p-2.5 rounded-[4px] transition-colors focus-visible:outline-none cursor-pointer ${isSelected ? 'bg-muted text-foreground' : 'bg-transparent hover:bg-muted text-foreground'}`}
                      >
      ...
      <div className={`flex-1 overflow-y-auto custom-scrollbar relative z-0 transition-all duration-150 ${importMenuOpen ? "opacity-30 pointer-events-none" : ""}`}>
                    ...
                    <div className="bg-card border border-border rounded-[8px] p-6 flex flex-col justify-center relative shadow-sm">
                       ...
                       <div className="flex justify-between items-end mb-3">
                         <div className="flex flex-col">
                           ...
                           <span className="text-[11px] text-muted-foreground mt-2 font-medium">Vault Liquid Value vs Wishlist Target Value</span>
                         </div>
                         <span className="text-[24px] font-black text-foreground">{goalProgress.toFixed(1)}%</span>
                       </div>
                       <div className="w-full h-3 bg-muted rounded-[2px] overflow-hidden border border-border">
                          <div className="h-full bg-foreground transition-all duration-1000 ease-out" style={{ width: `${goalProgress}%` }} />
                       </div>
                    </div>
                    <div className="bg-card border border-border rounded-[8px] p-6 flex flex-col shadow-sm">
                       ...
                       <div className="flex flex-col gap-2.5 flex-1 justify-center">
                          {top3Units.map((item, idx) => {
                            ...
                            return (
                              <div key={item.id} className="flex items-center justify-between p-2.5 rounded-[4px] bg-muted border border-border">
                                <div className="flex items-center gap-3 min-w-0 pr-2">
                                  <span className="font-mono font-black text-[12px] text-muted-foreground w-6 text-center shrink-0 py-0.5">#{idx + 1}</span>
                      ...
                      <div className="relative z-10">
                        <div className="flex items-center justify-between mb-2">
                          ...
                          <div className="flex items-center gap-1.5 text-[11px] font-bold font-mono">
                            {isSandbox ? (
                               <span className="text-muted-foreground bg-muted px-2 py-0.5 rounded-[2px] border border-border">Sandbox Value</span>
                              ...
                              <span className="text-muted-foreground bg-muted px-2 py-0.5 rounded-[2px] border border-border">Market Stable</span>
                            )}
                          </div>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-3 mt-4 pt-4 border-t border-border">
                          <div className="bg-muted p-3 rounded-[4px] border border-border flex flex-col">
                            ...
                            <span className="text-[14px] font-bold text-foreground font-mono mt-0.5">{isSandbox ? "0" : metrics.liquidValue.toLocaleString()}</span>
                          </div>
                          <div className="bg-muted p-3 rounded-[4px] border border-border flex flex-col">
                            <span className="text-[9px] font-bold text-muted-foreground uppercase tracking-wider">UNOB Proportion</span>
                            <span className="text-[14px] font-bold text-foreground font-mono mt-0.5">{isSandbox ? "0" : metrics.unobPercentage.toFixed(0)}% Unobtainable</span>
                          </div>
                        </div>
                      </div>
                      ...
                      {!isSandbox && (
                        <div className="flex flex-col gap-2 mt-5 relative z-10">
                          ...
                          <div className="w-full h-2 rounded-[2px] bg-muted overflow-hidden flex border border-border">
                            ...
                            <div className="h-full bg-foreground transition-all duration-700 ease-out" style={{ width: `${metrics.avgPct}%` }} title={`Average Demand: ${metrics.avgPct.toFixed(0)}%`} />
                            <div className="h-full bg-destructive transition-all duration-700 ease-out" style={{ width: `${metrics.lowPct}%` }} title={`Low Demand: ${metrics.lowPct.toFixed(0)}%`} />
                          </div>
                        ...
                        {!isSandbox && vaultView !== "wishlist" && (
                          <div className="hidden md:flex items-center gap-2 ml-4">
                            ...
                            <div className="w-20 h-1.5 bg-muted rounded-[2px] overflow-hidden border border-border">
                               <div className="h-full" style={{ width: `${progressPct}%`, backgroundColor: tierCfg.badgeColor }} />
                            </div>
                          </div>
                        )}
      ...
      {importMenuOpen && !isReadOnly && (
        <div className="absolute inset-0 z-[100000] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-black/80" onClick={() => setImportMenuOpen(false)} />
          <div className="bg-popover border border-border rounded-[8px] w-full max-w-lg shadow-[0_20px_60px_rgba(0,0,0,0.8)] relative z-10 flex flex-col overflow-hidden">
            ...
            <div className="flex items-center justify-between px-5 py-4 border-b border-border bg-card">
              ...
              <button onClick={() => setImportMenuOpen(false)} className="text-muted-foreground hover:text-foreground p-1 focus-visible:outline-none cursor-pointer"><X className="w-4 h-4" /></button>
            </div>
            ...
            <div className="p-5 flex flex-col gap-4">
              ...
              <div className="min-h-[60px] bg-muted rounded-[4px] p-3 border border-border flex flex-col gap-2">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[12px] font-mono font-bold text-foreground">{parsedImportItems.length} Found</span>
                </div>
                {parsedImportItems.length > 0 ? (
                  <div className="flex flex-wrap gap-1.5 mt-1">
                    {parsedImportItems.map((item, i) => (
                      <span key={i} className="text-[10px] font-bold bg-card text-foreground border border-border px-2 py-0.5 rounded-[2px] truncate max-w-[120px]">
            ...
            <div className="px-5 py-3.5 border-t border-border bg-card flex justify-end gap-2">
              <button onClick={() => setImportMenuOpen(false)} className="px-4 py-2 rounded-[4px] text-[12px] font-bold text-foreground hover:bg-muted border border-transparent focus-visible:outline-none cursor-pointer">Cancel</button>
              <button 
      ...
      {inspectTarget && (
        <div className="absolute inset-0 z-[100000] flex flex-col justify-end md:justify-center md:items-center p-0 md:p-4">
          <div className="absolute inset-0 bg-black/80" onClick={() => setInspectTarget(null)} />
          <div className="relative w-full md:max-w-xl max-h-[88vh] overflow-y-auto custom-scrollbar bg-popover rounded-t-[8px] md:rounded-[6px] p-5 shadow-2xl border-t md:border border-border z-10 flex flex-col gap-4">
            ...
            <div className="flex items-center justify-between border-b border-border pb-3 mt-2 md:mt-0">
              <div className="flex items-center gap-3.5 min-w-0 pr-4">
                <div className="w-12 h-12 rounded-[2px] overflow-hidden bg-muted border border-border shrink-0 relative flex items-center justify-center">
                  ...
                  {getProxyImage(inspectTarget.master.id, inspectTarget.master.imageUrl) && (
                    <img src={getProxyImage(inspectTarget.master.id, inspectTarget.master.imageUrl)!} alt="" className="absolute inset-0 w-full h-full object-cover z-10 bg-transparent" onError={(e) => handleImageError(e, inspectTarget.master.id)} />
                  )}
                </div>
                <div className="flex flex-col min-w-0">
                  <div className="flex items-center gap-2.5 flex-wrap">
                    ...
                    {inspectTarget.master.status && (
                      <span className="text-[10px] font-bold uppercase px-2 py-0.5 rounded-[2px] border tracking-wider inline-flex items-center gap-1" style={{ backgroundColor: GRID_STATUS_CFG[inspectTarget.master.status as keyof typeof GRID_STATUS_CFG]?.bg, color: GRID_STATUS_CFG[inspectTarget.master.status as keyof typeof GRID_STATUS_CFG]?.color, borderColor: GRID_STATUS_CFG[inspectTarget.master.status as keyof typeof GRID_STATUS_CFG]?.border }}>
                        <StatusIcon status={inspectTarget.master.status} />
                        {GRID_STATUS_CFG[inspectTarget.master.status as keyof typeof GRID_STATUS_CFG]?.label}
                      </span>
                    )}
                  </div>

                  <div className="flex items-center gap-2 mt-1 flex-wrap">
                    <span className="text-muted-foreground text-[11px] sm:text-[12px] font-bold uppercase tracking-wider">{inspectTarget.master.subtitle || "Official Unit"}</span>
              ...
              <button onClick={() => setInspectTarget(null)} className="p-1 text-muted-foreground hover:text-foreground transition-colors focus-visible:outline-none cursor-pointer self-start">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <div className="bg-card border border-border rounded-[4px] p-3 flex flex-col">
                ...
                {vaultView !== "wishlist" && (
                  <span className="text-[11px] text-muted-foreground mt-1 font-medium">Vault Subtotal: <strong className="text-foreground font-mono">{(getUnitConservativeValue(inspectTarget.master) * inspectTarget.item.quantity).toLocaleString()}</strong></span>
                )}
              </div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <div className="bg-card border border-border rounded-[4px] p-3 flex flex-col">
                  ...
                  <span className="text-[14px] font-black font-mono text-foreground mt-1">{inspectTarget.master.rarity ?? "N/A"}</span>
                </div>
                <div className="bg-card border border-border rounded-[4px] p-3 flex flex-col">
              ...
              {!isReadOnly && (
                vaultView === "wishlist" ? (
                  <div className="grid grid-cols-2 gap-2 pt-1">
                    ...
                    <button onClick={() => handleRemove(inspectTarget.item, inspectTarget.master)} className="flex items-center justify-center gap-2 py-2 rounded-[4px] text-[12px] font-bold text-muted-foreground hover:text-destructive bg-muted hover:bg-destructive/10 border border-border hover:border-destructive transition-colors focus-visible:outline-none cursor-pointer">
                  ...
                  <div className="grid grid-cols-2 gap-2 pt-1">
                    <button onClick={() => handleTogglePin(inspectTarget.item.unit_id, inspectTarget.item.is_pinned)} className={`flex items-center justify-center gap-2 py-2 rounded-[4px] text-[12px] font-bold border focus-visible:outline-none cursor-pointer transition-colors ${inspectTarget.item.is_pinned ? 'bg-destructive/10 text-destructive border-destructive' : 'bg-muted text-foreground border-border hover:bg-card hover:border-muted-foreground'}`}>
                    ...
                    <button onClick={() => handleRemove(inspectTarget.item, inspectTarget.master)} className="flex items-center justify-center gap-2 py-2 rounded-[4px] text-[12px] font-bold text-muted-foreground hover:text-foreground bg-muted hover:bg-card border border-border hover:border-muted-foreground transition-colors focus-visible:outline-none cursor-pointer">
                      <Trash2 className="w-3.5 h-3.5" />
                      <span>Remove</span>
                    </button>
                  </div>
      ...
      {toast && (
        <div className="absolute bottom-6 left-1/2 transform -translate-x-1/2 z-[1000] w-max max-w-[90vw]">
          <div className={`bg-popover border px-4 py-2.5 rounded-[6px] shadow-2xl flex items-center gap-4 ${toast.isError ? 'border-destructive' : 'border-border'}`}>
            <span className="text-[13px] font-bold text-foreground">
              {toast.message}
            </span>
            {toast.itemToRestore && !isReadOnly && (
              <button onClick={handleUndo} className="text-[12px] font-bold text-foreground hover:underline focus-visible:outline-none cursor-pointer">Undo</button>
            )}
            <button onClick={() => setToast(null)} className="text-muted-foreground hover:text-foreground focus-visible:outline-none p-0.5 cursor-pointer">
