// ================================================
// FILE: src/app/components/WelcomeModal.tsx
// ================================================

import { useState, useEffect } from "react";
import { Check, Palette, MousePointer2, Maximize, ArrowRight, ShieldCheck, Sparkles, BookOpen, ArrowLeft } from "lucide-react";
import { useLayoutStore, ThemeMode, BootChannel } from "../../store/useLayoutStore";
import { triggerHaptic } from "../../data/helpers";

export function WelcomeModal() {
  const [isVisible, setIsVisible] = useState(false);
  const [step, setStep] = useState<"legal" | "personalization" | "crossroads">("legal");
  const [legalView, setLegalView] = useState<"tos" | "pp" | null>(null);
  const [hasConsented, setHasConsented] = useState(false);

  const { 
    theme, setTheme, 
    globalCompactMode, setGlobalCompactMode, 
    bootChannel, setBootChannel 
  } = useLayoutStore();
        ...
        <div className="px-5 py-3.5 bg-popover border-b border-border flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[11px] font-black uppercase tracking-widest text-muted-foreground">
              ...
              {!legalView && step === "legal" && "Step 1 of 3: Terms & Compliance"}
              {!legalView && step === "personalization" && "Step 2 of 3: Interface Setup"}
              {!legalView && step === "crossroads" && "Step 3 of 3: Experience Choice"}
            </span>
          </div>
          {!legalView && (
            <span className="text-[11px] font-mono font-bold text-muted-foreground">
              {step === "legal" ? "1/3" : step === "personalization" ? "2/3" : "3/3"}
            </span>
          )}
        </div>
        ...
        {legalView && (
          <div className="flex flex-col h-[60vh] max-h-[500px]">
            <div className="flex-1 overflow-y-auto custom-scrollbar p-6 bg-card text-[13px] text-muted-foreground leading-relaxed flex flex-col gap-4">
              {legalView === "tos" ? (
                <>
                  <h3 className="text-[18px] font-black text-foreground mb-2">Terms of Service</h3>
                  <p><strong className="text-foreground">1. Non-Affiliation:</strong> ASTD Value List is a community-driven resource. We are not affiliated, associated, authorized, endorsed by, or in any way officially connected with Roblox Corporation or the creators of All Star Tower Defense.</p>
                  <p><strong className="text-foreground">2. Estimates Only:</strong> All unit values, demand ratings, and status tags are community estimates and are provided for informational purposes only. We are not responsible for any "bad trades."</p>
                  <p><strong className="text-foreground">3. API Usage:</strong> You agree not to use automated scripts, bots, or scrapers to continuously ping our API endpoints. Excessive requests will result in a ban.</p>
                </>
              ) : (
                <>
                  <h3 className="text-[18px] font-black text-foreground mb-2">Privacy Policy</h3>
                  <p><strong className="text-foreground">1. No PII Collection:</strong> We do not require account creation for core features. We do not ask for your Roblox password or any sensitive personal data.</p>
                  <p><strong className="text-foreground">2. Local Storage:</strong> Your active trade configurations, custom dictionary slang, and UI preferences are stored locally on your device using localStorage and IndexedDB.</p>
                  <p><strong className="text-foreground">3. Analytics:</strong> We use Google Analytics with Consent Mode v2. By accepting these terms, you allow tracking cookies to monitor page views and engagement time.</p>
                </>
              )}
            </div>
            <div className="p-4 border-t border-border bg-popover shrink-0">
              <button 
                onClick={() => { triggerHaptic('light'); setLegalView(null); }}
          ...
          <div className="p-5 md:p-7 flex flex-col gap-5">
            <div>
              ...
              <p className="text-[13px] text-muted-foreground mt-1.5 leading-relaxed">
                Before utilizing our live market values, trade calculator, and automated parsing tools, please verify your consent.
              </p>
            </div>
            ...
            <div className="bg-popover rounded-[6px] p-4 border border-border flex flex-col gap-2.5 text-[12px] text-muted-foreground leading-relaxed">
              ...
              <p>
                All values and ratings are real-time community-driven market estimates. We are not officially affiliated with Roblox Corporation or All Star Tower Defense.
              </p>
              <p>
                We use strict client-side encryption and caching. Your active trade configurations and custom slang are stored locally on your device.
              </p>
            </div>
            ...
            <label className="flex items-start gap-3 cursor-pointer group select-none pt-1">
              ...
              <span className="text-[12px] text-muted-foreground leading-relaxed">
                I accept the{" "}
                <button type="button" onClick={() => { triggerHaptic('light'); setLegalView("tos"); }} className="text-primary hover:underline font-bold">
                  Terms of Service
                </button>
                ,{" "}
                <button type="button" onClick={() => { triggerHaptic('light'); setLegalView("pp"); }} className="text-primary hover:underline font-bold">
                  Privacy Policy
                </button>
          ...
          <div className="p-5 md:p-7 flex flex-col gap-5">
            <div>
              ...
              <p className="text-[13px] text-muted-foreground mt-1 leading-relaxed">
                Pick your aesthetic and default workflow. You can modify these anytime in Profile Preferences.
              </p>
            </div>
            ...
            <div className="flex flex-col gap-2">
              ...
              <div className="grid grid-cols-3 gap-2">
                {[
                  ...
                  >
                    <span className="text-[12px] font-bold text-foreground leading-tight">{t.label}</span>
            ...
            <div className="flex flex-col gap-2">
              ...
              <div className="grid grid-cols-2 gap-2">
                {[
                  { id: 'value-list', label: "Value List", desc: "Immediate data" },
                  { id: 'trading-ads', label: "Trading Ads", desc: "Live trades" },
                  { id: 'home', label: "Home", desc: "Patch notes" },
                  { id: 'last-used', label: "Last Used", desc: "Remember state" },
                ].map(b => (
                  <button
                    key={b.id}
                    onClick={() => { triggerHaptic('light'); setBootChannel(b.id as BootChannel); }}
                    className={`p-2.5 rounded-[6px] border text-left flex flex-col transition-all cursor-pointer focus-visible:outline-none ${
                      bootChannel === b.id 
                        ? 'bg-popover border-primary ring-1 ring-primary/40 shadow-sm' 
                        : 'bg-muted border-border hover:bg-popover'
                  ...
                  >
                    <span className="text-[12px] font-bold text-foreground leading-tight">{b.label}</span>
                    <span className="text-[10px] text-muted-foreground mt-0.5 truncate">{b.desc}</span>
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="flex items-center justify-between p-3 rounded-[6px] bg-muted border border-border">
              <div className="flex flex-col pr-2">
                <span className="text-[12px] font-bold text-foreground flex items-center gap-1.5">
                  <Maximize className="w-3.5 h-3.5 text-primary" /> Compact UI Mode
                </span>
                <span className="text-[11px] text-muted-foreground">Tighter padding across tables and listings</span>
              </div>
              <button
                ...
                className={`relative w-11 h-6 rounded-full transition-colors shrink-0 cursor-pointer focus-visible:outline-none ${
                  globalCompactMode ? 'bg-primary' : 'bg-popover border border-border'
                }`}
              >
                <div className={`absolute top-0.5 left-0.5 bg-background w-5 h-5 rounded-full transition-transform shadow-sm ${
          ...
          <div className="p-5 md:p-7 flex flex-col gap-6">
            <div className="flex items-start gap-4 bg-muted/60 p-4 rounded-[8px] border border-border">
              ...
              <div className="flex flex-col">
                <div className="flex items-center gap-2">
                  <span className="text-[14px] font-bold text-foreground">Goddess Aqua</span>
                ...
                <p className="text-[12.5px] text-muted-foreground italic mt-1.5 leading-relaxed font-medium">
                  "Look at you, finally making it through the legal disclaimer. Before you go throwing your inventory away on terrible trades: do you want my divine guidance on how this platform works, or are you going to arrogantly wing it?"
                </p>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-2.5">
              ...
              >
                <BookOpen className="w-3.5 h-3.5" /> I Know What I'm Doing (Skip to App)
              </button>
            </div>
          </div>
        )}
