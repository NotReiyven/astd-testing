// ================================================
// FILE: src/app/components/TradeAnalyzer/smartParser.ts
// ================================================
...
export const parseSmartTradeAsync = (smartInput: string, ALL_UNITS: MasterUnit[]): Promise<ParseResult> => {
    return new Promise((resolve) => {
        if (!smartInput || !smartInput.trim()) {
            resolve({ giveCards: [], getCards: [], ambiguous: [], error: "Input is empty." });
            return;
        }

        if (ALL_UNITS.length === 0) {
            resolve({ giveCards: [], getCards: [], ambiguous: [], error: "Units database is loading..." });
            return;
        }
