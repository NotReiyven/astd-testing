  ...
  ({
    ...
    const obtainability = (() => {
      ...
      if (
        ...
        return "UNOB";
      if (note.includes("(obtainable)") || note.includes("[obtainable]"))
        return "OBN";
    ...
    const handleDragStart = (e: React.DragEvent) => {
      ...
      const numericValue =
        typeof master.value === "number" ? master.value : master.valueMin || 0;
      const popupUnit = {
        id: master.id,
        name: master.name,
    ...
    const liqDisplay =
      liqString.toLowerCase() === "black marketed"
        ? "BM"
        : liqString.toUpperCase();
    const rarityDisplay =
      (Number(master.rarity) || 0) % 1 === 0
        ? String(master.rarity || 0)
        : Number(master.rarity).toFixed(1);
    ...
    return (
      ...
      >
        {!isSelectMode && !isSandbox && !isReadOnly && !isWishlist && (
          ...
          >
            <button
              onClick={() => onQtyChange(item.unit_id, 1)}
              className="w-6 h-6 flex items-center justify-center bg-muted hover:bg-primary/20 hover:text-primary rounded-[4px] transition-colors focus-visible:outline-none"
            >
              <Plus className="w-4 h-4" />
            </button>
            <span className="font-mono font-bold text-[11px] text-foreground py-1">
              {item.quantity}
            </span>
            <button
              onClick={() => onQtyChange(item.unit_id, -1)}
              className="w-6 h-6 flex items-center justify-center bg-muted hover:bg-destructive/20 hover:text-destructive rounded-[4px] transition-colors focus-visible:outline-none"
            >
