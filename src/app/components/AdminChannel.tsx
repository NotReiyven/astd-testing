...
export function AdminChannel() {
  ...
  const [modActionData, setModActionData] = useState<{
    type: string;
    label: string;
  ...
  const executeModAction = () => {
    ...
    switch (modActionData.type) {
      case "purge_ads":
        handlePurgeAds(modReason);
        break;
      case "reset_profile":
        handleResetProfile(modReason);
        break;
      case "purge_comments":
        handlePurgeComments(modReason);
        break;
      case "wipe_inv":
        handleWipeInventory(modReason);
        break;
      case "wipe_wishlist":
        handleWipeWishlist(modReason);
        break;
      case "nuke":
        handleTotalAccountNuke(modReason);
        break;
      case "ban":
        updateUserRole(selectedUser.id, "banned", modReason);
        break;
      case "role":
        if (modActionData.payload)
          updateUserRole(selectedUser.id, modActionData.payload, modReason);
        break;
    }
              ...
              <div className="flex flex-col sm:flex-row gap-4">
                ...
                <div className="w-full sm:w-[110px] flex flex-col gap-1.5">
                  ...
                  <input
                    ...
                    value={newRoleRank}
                    onChange={(e) =>
                      setNewRoleRank(parseInt(e.target.value) || 10)
                    }
                    min={3}
                    max={98}
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar p-2.5">
              {isLoading ? (
                ...
                <div className="text-center text-muted-foreground text-[13px] mt-12 px-4">
                  ...
                  {users.map((u) => {
                    ...
                    return (
                      <button
                        ...
                        } ${isBanned && !isSelected ? "opacity-60" : ""}`}
                      >
                            ...
                            {u.assigned_roles?.map((roleName) => {
                              ...
                              return (
                                <span
                                  ...
                                  style={{
                                    backgroundColor: matchedRole
                                      ? `${matchedRole.color}15`
                                      : "transparent",
                                    borderColor: matchedRole
                                      ? `${matchedRole.color}40`
                                      : "transparent",
                                    color: matchedRole
                                      ? matchedRole.color
                                      : "inherit",
            ...
            {!selectedUser ? (
              <div className="flex-1 flex flex-col items-center justify-center text-center p-6">
                ...
                <p className="text-[13px] text-muted-foreground/70 mt-1 max-w-xs">
                  Select a user from the directory to review profiles, inspect
                  vaults, or execute moderation tools.
                </p>
              </div>
            ) : (
              <div className="flex-1 overflow-y-auto custom-scrollbar animate-fade-in pb-12">
                ...
                <div
                  className={`relative h-[130px] border-b border-border ${
                ...
                <div className="mt-12 px-6 flex flex-col">
                  <div className="flex flex-wrap items-center gap-3">
                    ...
                    {selectedUser.assigned_roles?.map((roleName) => {
                      const rStyle = availableRoles?.find(
                        (r) => r.name === roleName
                      );
                      return (
                        <span
                  ...
                  <div className="flex items-center gap-2 mt-1 w-fit group">
                    ...
                    <button
                      onClick={() => handleCopyId(selectedUser.discord_id)}
                      className="opacity-0 group-hover:opacity-100 transition-opacity text-muted-foreground hover:text-foreground cursor-pointer focus-visible:outline-none"
                    ...
                    <div className="bg-card backdrop-blur-sm rounded-lg p-4.5 flex flex-col border border-border shadow-md">
                      ...
                      {userIntel.isLoading ? (
                        <div className="h-[60px] flex items-center gap-3 text-muted-foreground text-[13px] font-medium animate-pulse">
                          <Activity className="w-4 h-4 text-primary" /> Fetching
                          vault records...
                        </div>
                      ) : (
                    ...
                    <div className="bg-card backdrop-blur-sm rounded-lg p-4.5 flex flex-col border border-border shadow-md gap-3">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {availableRoles
                          ?.filter(
                            (role) =>
                              role.name !== "banned" && role.name !== "master"
                          )
                          .map((r) => {
                            const isBanned =
                              selectedUser.assigned_roles?.includes("banned");
                            const disabled =
                              ...
                              (profile?.role !== "master" &&
                                (r.name === "master" || r.name === "admin")) ||
                              (profile?.role !== "master" &&
                                selectedUser.assigned_roles?.includes(
                                  "master"
                                ));

                            const isAssigned =
                              selectedUser.assigned_roles?.includes(r.name);
                            ...
                            return (
                              <button
                                ...
                                title={
                                  isBanned
                                    ? "Cannot modify roles of a banned user. Revoke ban first."
                                    : ""
                                }
                              >
                                {isAssigned && (
                            ...
                            <button
                              onClick={() =>
                                setModActionData({
                                  type: "ban",
                                  payload: "user",
                                  label: "Revoke Ban & Restore Access",
                                  isDestructive: false,
                                })
                              }
                          ...
                          <div className="flex flex-col gap-2.5 max-h-[320px] overflow-y-auto custom-scrollbar pr-1">
                            {modLogs.map((log) => (
                              ...
                              >
                                ...
                                <span className="text-[10px] text-muted-foreground mt-0.5 text-right italic">
        ...
        <div className="absolute bottom-6 right-6 z-[100000] animate-slide-up">
          ...
          >
            ...
            <button
              ...
              className="ml-2 text-muted-foreground hover:text-foreground cursor-pointer focus-visible:outline-none"
            >
