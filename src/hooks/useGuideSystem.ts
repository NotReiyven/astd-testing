// ================================================
// FILE: src/hooks/useGuideSystem.ts
// ================================================
...
interface UseGuideSystemProps {
  ...
  setTutorialTab: (tab: "sandbox" | "simulator" | "theory" | "dictionary") => void;
  bootStage: string;
}
...
export function useGuideSystem({ 
  ...
  const [guideState, setGuideState] = useStickyState<{ type: GuideType; step: number }>(
    { type: null, step: 0 },
    "astd_active_guide",
    (v): v is { type: GuideType; step: number } => typeof v === "object" && v !== null && 'step' in v
  );
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      const isAcknowledged = localStorage.getItem("astd_welcome_acknowledged") === "true";
      if (!isAcknowledged) {
  ...
  useEffect(() => {
    if (bootStage !== 'complete' || !profile || hasTriggeredAuthTour.current) return;
