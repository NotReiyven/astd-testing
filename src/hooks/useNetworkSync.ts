...
export function useNetworkSync() {
  useEffect(() => {
    ...
    const handleOnline = async () => {
      ...
      const processQueue = async (storageKey: string, processor: (item: any) => Promise<void>) => {
        ...
        for (const item of batch) {
          ...
          if (item.userProfile?.id && item.userProfile.id !== currentUserId) continue;
      ...
      await processQueue('astd_offline_inventory', async (op) => {
        if (op.action === 'UPSERT') {
          ...
          if (op.item.user_id === currentUserId) await supabase.from('user_inventory').insert(op.item);
